package SQL::Transpose::Generator::DDL::SQLServer;

=head1 NAME

SQL::Transpose::Generator::DDL::SQLServer - A Moo based MS SQL Server DDL
generation engine.
...
with 'SQL::Transpose::Generator::Role::DDL';
...
sub unique_constraint_single {
    ...
    'CONSTRAINT ' . $self->unique_constraint_name($constraint) . ' UNIQUE (' . join(', ', map $self->quote($_), $constraint->fields) . ')';
}
...
sub constraints {
    ...
    (map $self->enum_constraint($_->name, {$_->extra}->{list} || []), grep { 'enum' eq lc $_->data_type } $table->get_fields),
...
sub remove_table_constraints {
    ...
    my $name   = $table->name;
    my $q_name = $self->quote($name);
    "IF EXISTS (SELECT name FROM sysobjects WHERE name = '$name' AND type = 'U')" . " ALTER TABLE $q_name NOCHECK CONSTRAINT all;";
}
...
sub drop_tables {
    ...
    if ($self->add_drop_table) {
        ...
        return join "\n", (
            ($self->add_comments ? ('--', '-- Turn off constraints', '--', '',) : ()),
