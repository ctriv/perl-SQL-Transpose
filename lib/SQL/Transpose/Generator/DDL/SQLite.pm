package SQL::Transpose::Generator::DDL::SQLite;

=head1 NAME

SQL::Transpose::Generator::DDL::SQLite - A Moo based SQLite DDL generation
engine.
...
sub field_autoinc {
    ...
    return (
        (($field->extra->{auto_increment_type} || '') eq 'monotonic' and $self->_ipk($field) and $field->is_auto_increment)
        ? 'AUTOINCREMENT'
        : ''
    );
}
