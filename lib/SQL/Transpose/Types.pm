...
SQL::Transpose::Types - Type checking functions
...
C<isa> type checks.
Errors are reported using L<SQL::Transpose::Utils/throw>.

=cut

use SQL::Transpose::Utils qw(throw);
...
class C<< SQL::Transpose::Schema::I<$type> >>.
...
sub schema_obj {
    my ($class) = @_;
    my $name = lc $class;
    $class = 'SQL::Transpose::Schema' . ($class eq 'Schema' ? '' : "::$class");
    return sub {
