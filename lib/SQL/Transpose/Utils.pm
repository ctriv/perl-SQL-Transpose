...
sub ddl_parser_instance {
    ...
    eval "require SQL::Transpose::Parser::$type"
        or die "Unable to load grammar-spec container SQL::Transpose::Parser::$type:\n$@";
...
sub throw {
    die SQL::Transpose::Utils::Error->new($_[0]);
}
...
SQL::Transpose::Utils - SQL::Transpose Utility functions
...
C<SQL::Transpose::Utils> contains utility functions designed to be
used from the other modules within the C<SQL::Transpose> modules.
...
Will warn
  ...
  [SQL::Transpose: Error reading file at foo/666]
...
A more useful example, from the C<SQL::Transpose::Parser::Excel> test
suite:
...
this use).  For example, this:
  ...
  use SQL::Transpose::Utils qw(header_comment $DEFAULT_COMMENT);
...
Used by both L<Parser::MySQL|SQL::Transpose::Parser::MySQL> and
L<Producer::MySQL|SQL::Transpose::Producer::MySQL> in order to provide a
...
L<batch_alter_table|SQL::Transpose::Diff/batch_alter_table($table, $hash, $args) (optional)>
