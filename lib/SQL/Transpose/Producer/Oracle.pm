package SQL::Transpose::Producer::Oracle;

=head1 NAME

SQL::Transpose::Producer::Oracle - Oracle SQL producer

=head1 SYNOPSIS

  use SQL::Transpose;

  my $t = SQL::Transpose->new( parser => '...', producer => 'Oracle' );
...
0.09002 of L<SQL::Transpose> when called in array context. In scalar
...
sub create_field {
    my ($self, $field, $options, $field_name_scope) = @_;
    my $qf = $options->{quote_field_names};
    my $qt = $options->{quote_table_names};
...
sub create_view {
    ...
    my $qt = $options->{quote_table_names};
    my $view_name = $self->quote($view->name, $qt);
...
SQL::Transpose, DDL::Oracle, mysql2ora.
