package SQL::Transpose::Producer::DB2;

=head1 NAME

SQL::Transpose::Producer::DB2 - DB2 SQL producer

=head1 SYNOPSIS

  use SQL::Transpose;

  my $t = SQL::Transpose->new( parser => '...', producer => 'DB2' );
...
sub produce {
    ...
    foreach my $table ($schema->get_tables) {
        ...
        my ($table_def, $fks) = $self->create_table($table, {no_comments => $no_comments});
...
sub create_field {
    ...
    $field_def .= $field->is_auto_increment                    ? ' GENERATED BY DEFAULT AS IDENTITY (START WITH 1, INCREMENT BY 1)' : '';
    ...
    $field_def .=
        ...
        : $field->default_value =~ /current( |_)timestamp/i || $field->default_value =~ /\Qnow()\E/i ? ' DEFAULT CURRENT TIMESTAMP'
        : defined $field->default_value ? (" DEFAULT " . ($data_type =~ /(INT|DOUBLE)/i ? $field->default_value : "'" . $field->default_value . "'"))
        :                                 '';
...
sub add_field {
    ...
    my $out = sprintf('ALTER TABLE %s ADD COLUMN %s', $new_field->table->name, $self->create_field($new_field));
