package SQL::Transpose::Producer::HTML;
...
sub produce {
    ...
    if ($linktable) {
        ...
        push @html, $q->comment("Table listing ($count)"), $q->a({-name => 'top'}),
            $q->start_table(
            {
                -width => '100%',
                -class => 'LinkTable'
    ...
    foreach my $table ($schema->get_tables) {
        ...
        push @html,
            ...
            $q->Tr($q->th({-class => 'FieldHeader'}, ['Field Name', 'Data Type', 'Size', 'Default Value', 'Other', 'Foreign Key']));
        ...
        if (my @indices = $table->get_indices) {
            push @html, $q->h3('Indices'), $q->start_table({-border => 1}), $q->Tr({-class => 'IndexRow'}, $q->th(['Name', 'Fields']));
        ...
        if (@constraints) {
            push @html, $q->h3('Constraints'), $q->start_table({-border => 1}), $q->Tr({-class => 'IndexRow'}, $q->th(['Type', 'Fields']));
    ...
    if ($wrap) {
        push @html, qq[Created by <a href="http://sqlfairy.sourceforge.net">], qq[SQL::Transpose</a>], $q->end_html;
    }
...
SQL::Transpose::Producer::HTML - HTML producer for SQL::Transpose

=head1 SYNOPSIS

  use SQL::Transpose::Producer::HTML;
