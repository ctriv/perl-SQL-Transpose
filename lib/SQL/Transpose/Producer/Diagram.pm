package SQL::Transpose::Producer::Diagram;

=head1 NAME

SQL::Transpose::Producer::Diagram - ER diagram producer for SQL::Transpose
...
TABLE:
    foreach my $table_name (@table_names) {
        ...
        foreach my $fld_desc (@fld_desc) {
            ...
            push @imap_coords, [$imap_url . "#$table_name-$orig_name", $this_col_x, $y - $font->height, $length, $y_link,];
        }
            ...
            foreach my $c ($table->get_constraints) {
                ...
                foreach my $field_name ($c->fields) {
                    foreach my $fk_field ($c->reference_fields) {
                        ...
                        push @fk_registry, [[$fk_table, $fk_field], [$table_name, $field_name],];
                    }
                }
            }
        }
        ...
        push @shapes, ['line', $this_col_x - 5, $below_table_name, $this_max_x, $below_table_name, 'black'];
        my @bounds = ($this_col_x - 5, $top - 5, $this_max_x, $y + 5);
        if ($add_color) {
            unshift @shapes, ['filledRectangle', $bounds[0], $bounds[1], $this_max_x, $below_table_name, 'khaki'];
            unshift @shapes, ['filledRectangle', @bounds, 'white'];
        }

        push @imap_coords, [$imap_url . "#$table_name", $bounds[0], $bounds[1], $this_max_x, $below_table_name,];
    ...
    unless ($no_lines) {
        ...
        if ($natural_join) {
            ...
            foreach my $pair (@fk_registry) {
                push @position_bunches, [$coords{$pair->[0][0]}{$pair->[0][1]}{'coords'}, $coords{$pair->[1][0]}{$pair->[1][1]}{'coords'},];
            }
        }

        my $is_directed = $natural_join ? 0 : 1;

        foreach my $bunch (@position_bunches) {
            my @positions = @$bunch;
            ...
            foreach my $i (0 .. $#positions) {
                ...
                foreach my $j (0 .. $#positions) {
                    ...
                    my @distances = ();
                    push @distances, [abs($ax - $cx) + abs($ay - $cy), [$ax, $ay, $cx, $cy], ['left',  'left']];
                    push @distances, [abs($ax - $dx) + abs($ay - $dy), [$ax, $ay, $dx, $dy], ['left',  'right'],];
                    push @distances, [abs($bx - $cx) + abs($by - $cy), [$bx, $by, $cx, $cy], ['right', 'left'],];
                    push @distances, [abs($bx - $dx) + abs($by - $dy), [$bx, $by, $dx, $dy], ['right', 'right'],];
                    ...
                    if ($is_directed) {
                        if (   $side1 eq 'right' && $side2 eq 'left'
                            || $side1 eq 'left' && $side2 eq 'left') {
                            push @shapes, ['line', $x2 - 3, $y2 - 3, $x2, $y2, 'cadetblue'];
                            push @shapes, ['line', $x2 - 3, $y2 + 3, $x2, $y2, 'cadetblue'];
                            push @shapes, ['line', $x2 - 3, $y2 - 3, $x2 - 3, $y2 + 3, 'cadetblue'];
                        }
                        else {
                            push @shapes, ['line', $x2 + 3, $y2 - 3, $x2, $y2, 'cadetblue'];
                            push @shapes, ['line', $x2 + 3, $y2 + 3, $x2, $y2, 'cadetblue'];
    ...
    if (%legend) {
        ...
        my $longest;
        foreach my $len (map { length $_ } values %legend) {
