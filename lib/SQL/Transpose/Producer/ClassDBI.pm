package SQL::Transpose::Producer::ClassDBI;
...
sub produce {
    ...
    my $dsn = $args->{'dsn'} || sprintf('dbi:%s:_', $CDBI_auto_pkgs{$parser_type} ? $CDBI_auto_pkgs{$parser_type} : $parser_type);
    my $sep = '# ' . '-' x 67;
    ...
    foreach my $table ($schema->get_tables) {
        ...
        if ($is_data) {
            foreach my $link (keys %{$linkable{$table_name}}) {
                ...
                if (my $fk_xform = $t->format_fk_name) {
                    ...
                    $linkmethodname = $fk_xform->($linkable{$table_name}{$link}->name, ($schema->get_table($link)->primary_key->fields)[0]) . 's';
                }
                else {
                    # ADD CALLBACK FOR PLURALIZATION MANGLING HERE
                    $linkmethodname = $linkable{$table_name}{$link}->name . '_' . ($schema->get_table($link)->primary_key->fields)[0] . 's';
                }
                ...
                foreach my $field ($linkable{$table_name}{$link}->get_fields) {
                    ...
                    next
                        unless ($field->foreign_key_reference->reference_table eq $table_name
        ...
        foreach my $field ($table->get_fields) {
            if ($field->is_foreign_key) {
                ...
                my $fk_method = join('::', $table_pkg_name, $t->format_fk_name($table_name, $field_name));
...
SQL::Transpose::Producer::ClassDBI - create Class::DBI classes from schema

=head1 SYNOPSIS

Use this producer as you would any other from SQL::Transpose.  See
L<SQL::Transpose> for details.

This package uses SQL::Transpose's formatting methods
