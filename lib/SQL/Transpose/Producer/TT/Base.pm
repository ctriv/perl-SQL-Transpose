package SQL::Transpose::Producer::TT::Base;

=pod

=head1 NAME

SQL::Transpose::Producer::TT::Base - TT (Template Toolkit) based Producer base
class.
...
=head1 SYNOPSIS
 ...
 package SQL::Transpose::Producer::Foo;
...
You can find an introduction to this module in L<SQL::Transpose::Manual>.

The 1st thing the module does is convert the produce sub routine call we get
from SQL::Transpose into a method call on an object, which we can then
...
Called with the L<SQL::Transpose::Schema> object and should return one (it
...
L<SQL::Transpose> object running the translation. Dies if this is not given.

=head2 translator

Return the L<SQL::Transpose> object.

=head2 schema

Return the L<SQL::Transpose::Schema> we are translating. This is equivalent
...
- Merge in L<TT::Table|SQL::Transpose::Producer::TT::Table>.
