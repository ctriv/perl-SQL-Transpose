...
sub produce {
    ...
    return Dump(
        {
            schema => {
                tables     => {map { ($_->name => $self->view_table($_)) } $schema->get_tables,},
                views      => {map { ($_->name => $self->view_view($_)) } $schema->get_views,},
                triggers   => {map { ($_->name => $self->view_trigger($_)) } $schema->get_triggers,},
                procedures => {map { ($_->name => $self->view_procedure($_)) } $schema->get_procedures,},
            },
            translator => {
