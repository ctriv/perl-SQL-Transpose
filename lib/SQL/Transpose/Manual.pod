...
SQL::Transpose::Manual - sqlfairy user manual

=head1 SYNOPSIS

SQL::Transpose (AKA "SQLFairy") is a collection of modules for
...
read the documentation for L<SQL::Transpose>.
...
interfaces to the SQL::Transpose code.  All SQLFairy scripts begin
...
the names of the Parser and Producer classes in SQL::Transpose.  For
...
=head1 DOCUMENTING WITH SQL::Transpose
...
subroutine that SQL::Transpose can call for the production of the
output.  This subroutine will be passed a single argument of the
SQL::Transpose object which you can use to access the Schema objects.
Please read the POD for SQL::Transpose and SQL::Transpose::Schema to
...
A custom parser will be passed two arguments:  the SQL::Transpose
...
And here is the output produced by this script:
  ...
  --
  -- Created by SQL::Transpose::Producer::Oracle
...
SQLFairy :) If so, the SQL::Transpose::Producer::TT::Base producer may be
...
See L<SQL::Transpose::Producer::TT::Base> for more details.
