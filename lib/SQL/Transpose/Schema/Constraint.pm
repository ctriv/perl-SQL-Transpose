package SQL::Transpose::Schema::Constraint;

=pod

=head1 NAME

SQL::Transpose::Schema::Constraint - SQL::Transpose constraint object

=head1 SYNOPSIS

  use SQL::Transpose::Schema::Constraint;
  my $constraint = SQL::Transpose::Schema::Constraint->new(
      name   => 'foo',
      fields => [ id ],
      type   => PRIMARY_KEY,
  );
...
C<SQL::Transpose::Schema::Constraint> is the constraint object.
...
use SQL::Transpose::Utils qw(ex2err throw);
use SQL::Transpose::Role::ListAttr;
use SQL::Transpose::Types qw(schema_obj enum);
use Sub::Quote qw(quote_sub);

extends 'SQL::Transpose::Schema::Object';

# these are CONSTANTS, not strings....
my %VALID_CONSTRAINT_TYPE = (PRIMARY_KEY, 1, UNIQUE, 1, CHECK_C, 1, FOREIGN_KEY, 1, NOT_NULL, 1,);

=head2 new

Object constructor.

  my $schema           =  SQL::Transpose::Schema::Constraint->new(
...
=cut
    ...
    my $schema = $table->schema or return $self->error('Table ', $table->name, ' has no schema object');
    ...
    if ($type eq FOREIGN_KEY) {
        ...
        foreach my $ref_field (@ref_fields) {
...
around equals => sub {
    ...
    return 0 unless $case_insensitive
