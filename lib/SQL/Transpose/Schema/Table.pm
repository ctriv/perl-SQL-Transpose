package SQL::Transpose::Schema::Table;

=pod

=head1 NAME

SQL::Transpose::Schema::Table - SQL::Transpose table object

=head1 SYNOPSIS

  use SQL::Transpose::Schema::Table;
  my $table = SQL::Transpose::Schema::Table->new( name => 'foo' );

=head1 DESCRIPTION

C<SQL::Transpose::Schema::Table> is the table object.
...
use Moo;
use SQL::Transpose::Utils qw(parse_list_arg ex2err throw carp_ro);
use SQL::Transpose::Types qw(schema_obj);
use SQL::Transpose::Role::ListAttr;
use SQL::Transpose::Schema::Constants;
use SQL::Transpose::Schema::Constraint;
use SQL::Transpose::Schema::Field;
use SQL::Transpose::Schema::Index;

use Carp::Clan '^SQL::Transpose';
use List::Util 'max';
...
C<SQL::Transpose::Schema::Constraint> object.
  ...
  my $c2 = SQL::Transpose::Schema::Constraint->new( name => 'uniq' );
...
sub add_constraint {
    my $self             = shift;
    my $constraint_class = 'SQL::Transpose::Schema::Constraint';
    my $constraint;
    ...
    if ($pk && $constraint->type eq PRIMARY_KEY) {
        ...
        foreach my $fname ($constraint->fields) {
...
an index name or an C<SQL::Transpose::Schema::Constraint> object.
...
C<SQL::Transpose::Schema::Index> object.
  ...
  my $i2 = SQL::Transpose::Schema::Index->new( name => 'id' );
...
sub add_index {
    my $self        = shift;
    my $index_class = 'SQL::Transpose::Schema::Index';
    my $index;
    ...
    if (UNIVERSAL::isa($_[0], $index_class)) {
        ...
        my %args = @_;
        $args{'table'} = $self;
        $index = $index_class->new(\%args) or return $self->error($index_class->error);
    }
...
an index name of an C<SQL::Transpose::Schema::Index> object.
...
C<SQL::Transpose::Schema::Field> object.  The "name" parameter is
...
sub add_field {
    my $self        = shift;
    my $field_class = 'SQL::Transpose::Schema::Field';
    my $field;
    ...
    if (UNIVERSAL::isa($_[0], $field_class)) {
        ...
        my %args = @_;
        $args{'table'} = $self;
        $field = $field_class->new(\%args) or return $self->error($field_class->error);
    }
...
found and removed, an error otherwise. The single parameter can be either
a field name or an C<SQL::Transpose::Schema::Field> object.
...
sub is_valid {
    ...
    foreach my $object ($self->get_fields, $self->get_indices, $self->get_constraints) {
...
Returns the C<SQL::Transpose::Schema::Constraint> object representing
the primary key.
...
Get or append to the table's options (e.g., table types foreach mySQL).
