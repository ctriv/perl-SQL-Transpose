...
sub schema_diff {
    ...
    ## _schema isa SQL::Transpose::Schema
...
sub BUILD {
    ...
    if ($args->{producer_options}) {
        ...
        $self->producer_args({%{$args->{producer_options}}, %{$self->producer_args}});
    }
...
sub _compute_table_differences {
    ...
    foreach my $tar_table (@tar_tables) {
    ...
    foreach my $src_table ($source_schema->get_tables) {
        my $src_table_name = $src_table->name;
...
sub produce_diff_sql {
    ...
    if (!$self->no_batch_alters && $producer_class->can('batch_alter_table')) {
        ...
        foreach my $table (sort keys %{$self->table_diff_hash}) {
            ...
            push @diffs,
                $producer_class->batch_alter_table($tar_table, {map { $func_map{$_} => $self->table_diff_hash->{$table}{$_} } keys %func_map},
                $self->producer_args);
        }
    }
    else {
    ...
    if (my @tables_to_drop = @{$self->{tables_to_drop} || []}) {
        ...
        push @diffs,
              $meth ? (map { $producer_class->drop_table($_, $self->producer_args) } @tables_to_drop)
...
sub diff_table_indexes {
    ...
    my (%checked_indices);
INDEX_CREATE:
    foreach my $i_tar ($tar_table->get_indices) {
        foreach my $i_src ($src_table->get_indices) {
...
CONSTRAINT_CREATE:
    foreach my $c_tar ($tar_table->get_constraints) {
        foreach my $c_src ($src_table->get_constraints) {
...
sub diff_table_fields {
    ...
    foreach my $tar_table_field ($tar_table->get_fields) {
    ...
    foreach my $src_table_field ($src_table->get_fields) {
...
SQL::Transpose::Diff - determine differences between two schemas

=head1 DESCRIPTION

Takes two input SQL::Transpose::Schemas (or SQL files) and produces ALTER
...
Simplest usage:
 ...
 my $sql = SQL::Transpose::Diff::schema_diff($source_schema, 'MySQL', $target_schema, 'MySQL', $options_hash)
...
Hash of extra arguments passed to L<SQL::Transpose/new> and the below
