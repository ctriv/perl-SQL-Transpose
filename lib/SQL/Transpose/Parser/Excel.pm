package SQL::Transpose::Parser::Excel;

=head1 NAME

SQL::Transpose::Parser::Excel - parser for Excel
...
sub parse {
    ...
    foreach my $num (0 .. $wb_count - 1) {
        ...
        unless (defined $args->{'scan_fields'}
            ...
            for (my $iR = $ws->{'MinRow'} == 0 ? 1 : $ws->{'MinRow'}; defined $ws->{'MaxRow'} && $iR <= $ws->{'MaxRow'}; $iR++) {
...
Spreadsheet::ParseExcel, SQL::Transpose.
