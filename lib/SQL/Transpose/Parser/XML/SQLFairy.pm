package SQL::Transpose::Parser::XML::SQLFairy;

=head1 NAME

SQL::Transpose::Parser::XML::SQLFairy - parser for SQL::Transpose's XML.

=head1 SYNOPSIS

  use SQL::Transpose;

  my $translator = SQL::Transpose->new( show_warnings  => 1 );
...
project (L<SQL::Transpose>).  The XML must be in the XML namespace
C<http://sqlfairy.sourceforge.net/sqlfairy.xml>.
See L<SQL::Transpose::Producer::XML::SQLFairy> for details of this format.
...
L<SQL::Transpose::Schema::Field>.  Use empty quotes or 'EMPTY_STRING'
...
L<SQL::Transpose::Producer::XML::SQLFairy>.
...
sub parse {
    ...
    foreach my $tblnode (sort { ("" . $xp->findvalue('sqlf:order|@order', $a) || 0) <=> ("" . $xp->findvalue('sqlf:order|@order', $b) || 0) } @nodes)
    {
...
L<perl>, L<SQL::Transpose>, L<SQL::Transpose::Producer::XML::SQLFairy>,
L<SQL::Transpose::Schema>.
