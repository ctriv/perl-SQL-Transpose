package SQL::Transpose::Parser::Oracle;

=head1 NAME

SQL::Transpose::Parser::Oracle - parser for Oracle

=head1 SYNOPSIS

  use SQL::Transpose;
  use SQL::Transpose::Parser::Oracle;

  my $translator = SQL::Transpose->new;
  $translator->parser("SQL::Transpose::Parser::Oracle");
...
sub parse {
    ...
    my @triggers = sort { $result->{triggers}->{$a}->{'order'} <=> $result->{triggers}->{$b}->{'order'} } keys %{$result->{triggers}};
...
SQL::Transpose, Parse::RecDescent, DDL::Oracle.
