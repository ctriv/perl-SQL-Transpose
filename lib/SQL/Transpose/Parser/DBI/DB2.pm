package SQL::Transpose::Parser::DBI::DB2;

=head1 NAME

SQL::Transpose::Parser::DBI::DB2 - parser for DBD::DB2
    ...
    foreach my $table_info (@tables) {
        ...
        if ($table_info->{TYPE} eq 'T' && $table_info->{TABSCHEMA} !~ /^SYS/) {
            ...
            my $con = $table->add_constraint(
                name   => $c->{CONSTNAME},
                fields => \@fields,
                type   => $c->{TYPE} eq 'P' ? PRIMARY_KEY : $c->{TYPE} eq 'F' ? FOREIGN_KEY : UNIQUE
            ) || die $table->error;
            ...
            foreach my $ind (keys %$inds) {
                ...
                my @fields = map { $_->{INDNAME} eq $ind ? $_->{COLNAME} : () } (values %{$indcols});
                ...
                my $inew = $table->add_index(
                    name   => $index->{INDNAME},
                    fields => \@fields,
                    type   => $index->{UNIQUERULE} eq 'U' ? UNIQUE : NORMAL
                ) || die $table->error;
