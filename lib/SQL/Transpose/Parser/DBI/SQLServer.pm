package SQL::Transpose::Parser::DBI::SQLServer;

=head1 NAME

SQL::Transpose::Parser::DBI::SQLServer - parser for SQL Server through DBD::ODBC
...
ORDER BY o.name,
    ...
    foreach my $table_info (@tables) {
        ...
        if ($table_info->{TABLE_TYPE} eq "TABLE") {
            ...
            my $cols = $columns->{$table_info->{TABLE_CAT}}->{$table_info->{TABLE_SCHEM}}->{$table_info->{TABLE_NAME}}->{columns};
...
DBI, DBD::ODBC, SQL::Transpose::Schema.
