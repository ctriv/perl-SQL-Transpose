package SQL::Transpose::Parser::MySQL;

=head1 NAME

SQL::Transpose::Parser::MySQL - parser foreach mySQL
...
=head1 SYNOPSIS
  ...
  $translator->parser("SQL::Transpose::Parser::MySQL");
...
Valid version specifiers for C<mysql_parser_version> are listed L<here|SQL::Transpose::Utils/parse_mysql_version>
...
view_table_def : not_delimiter
    {
        ...
        my @tables;
        foreach my $tbl ( split( /\s*,\s*/, $clause ) ) {
...
sub parse {
    ...
    # Preprocess foreach mySQL-specific and not-before-version comments
    # from mysqldump
    my $parser_version = parse_mysql_version($translator->parser_args->{mysql_parser_version}, 'mysql') || DEFAULT_PARSER_VERSION;
    ...
    foreach my $table_name (@tables) {
        ...
        foreach my $fname (@fields) {
            ...
            foreach my $qual (qw[ binary unsigned zerofill list collate ], 'character set', 'on update') {
...
Parse::RecDescent, SQL::Transpose::Schema.
