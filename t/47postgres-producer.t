...
BEGIN {
    maybe_plan(undef, 'SQL::Transpose::Producer::PostgreSQL', 'Test::Differences',);
}
use Test::Differences;
use SQL::Transpose;

my $PRODUCER = sub {
    return SQL::Transpose::Producer::PostgreSQL->create_field(@_);
};

{
    ...
    my $field = SQL::Transpose::Schema::Field->new(
        name              => 'baz',
        comments          => ["multi\nline", 'single line'],
        table             => $table,
        data_type         => 'VARCHAR',
...
my $field_array = SQL::Transpose::Schema::Field->new(
    name              => 'myfield',
    table             => $table,
...
my $field1_2 = SQL::Transpose::Schema::Field->new(
    name              => 'myfield_2',
    table             => $table,
    data_type         => 'VARCHAR',
    size              => 10,
...
foreach my $name ('foo', undef) {
    ...
    if ($name) {
        is(
            $fk_constraint_fk_ref->[0], "ALTER TABLE mytable ADD CONSTRAINT $name FOREIGN KEY (myfield)
  REFERENCES mytable2 (myfield_2) DEFERRABLE", 'Create Foreign Key Constraint works'
        ...
        is(
            $fk_constraint_fk_ref->[0], 'ALTER TABLE mytable ADD FOREIGN KEY (myfield)
...
my $alter_field = SQL::Transpose::Producer::PostgreSQL->alter_field($field1, $field2);
is(
...
my $field_serial = SQL::Transpose::Schema::Field->new(
    name              => 'serial_field',
    table             => $table,
    data_type         => 'INT',
    is_auto_increment => 1,
...
my $field_bigserial = SQL::Transpose::Schema::Field->new(
...
$field_bigserial = SQL::Transpose::Schema::Field->new(
    name              => 'bigserial_field',
    table             => $table,
    data_type         => 'INT',
    is_auto_increment => 1,
...
my $field3 = SQL::Transpose::Schema::Field->new(
    name              => 'time_field',
    table             => $table,
    data_type         => 'TIME',
    default_value     => undef,
...
my $field3_datetime_with_TZ_sql = SQL::Transpose::Producer::PostgreSQL->create_field($field3_datetime_with_TZ);
...
my $field3_time_without_TZ_sql = SQL::Transpose::Producer::PostgreSQL->create_field($field3_time_without_TZ);
...
my $field_num = SQL::Transpose::Schema::Field->new(
    name      => 'num',
    table     => $table,
    data_type => 'numeric',
    size      => [10, 2],
...
my $field4 = SQL::Transpose::Schema::Field->new(
    name              => 'bytea_field',
    table             => $table,
    data_type         => 'bytea',
    size              => '16777215',
...
my $field5 = SQL::Transpose::Schema::Field->new(
    name              => 'enum_field',
    table             => $table,
    data_type         => 'enum',
...
$alter_field = SQL::Transpose::Producer::PostgreSQL->alter_field($field6, $field7);
...
my $field8 = SQL::Transpose::Schema::Field->new(
    name              => 'ts_field',
    table             => $table,
    data_type         => 'timestamp with time zone',
    size              => 6,
...
my $field9 = SQL::Transpose::Schema::Field->new(
    name              => 'time_field',
    table             => $table,
...
my $field10 = SQL::Transpose::Schema::Field->new(
    name              => 'interval_field',
    table             => $table,
    data_type         => 'interval',
    size              => 6,
...
my $field11 = SQL::Transpose::Schema::Field->new(
    name              => 'time_field',
    table             => $table,
    data_type         => 'time without time zone',
    size              => 6,
...
my $field12 = SQL::Transpose::Schema::Field->new(
    name              => 'time_field',
    table             => $table,
    data_type         => 'timestamp',
    is_auto_increment => 0,
...
my $field13 = SQL::Transpose::Schema::Field->new(
...

{
...

    {
        ...
        is($PRODUCER->($null_default), q{null_default_2 character varying(10) DEFAULT NULL}, 'default null from special cased string',);
    }

    {
        ...
        is($PRODUCER->($func_default), q{func_default character varying(10) DEFAULT func(funky)}, 'unquoted default from scalar ref',);
    }
}
