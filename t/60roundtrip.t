...
SKIP: {
    ...
    TODO: {
            ...
            lives_ok(sub { check_roundtrip($args, $base_schema, $args->{todo_cmp}) }, "Round trip for $args->{name} did not throw an exception",);
        }
    }
}
...
sub check_roundtrip {
    ...
    like($base_out, $string_re->{$args->{engine}} || $string_re->{SQL}, "Received some meaningful output from the first $args->{name} production",)
        or do {
    ...
    like($rt_out, $string_re->{$args->{engine}} || $string_re->{SQL}, "Received some meaningful output from the second $args->{name} production",)
        or do {
