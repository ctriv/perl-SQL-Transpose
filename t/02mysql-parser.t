...
#
{
    ...
    my $tableTypeFound = 0;
    my $charsetFound   = 0;
    foreach my $t1_option_ref ($table1->options) {
    ...
    is_deeply(\@options, ['ALGORITHM=UNDEFINED', 'DEFINER=`cmdomain`@`localhost`', 'SQL SECURITY DEFINER',], 'Only version 50013 options parsed',);
...
foreach my $target (keys %$parse_as) {
    foreach my $str (keys %{$parse_as->{$target}}) {
        cmp_ok(
...

{
    ...
    my $tableTypeFound = 0;
    my $unionFound     = 0;
    foreach my $t_option_ref ($table->options) {
